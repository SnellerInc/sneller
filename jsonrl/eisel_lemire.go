...
// lifted from the Go strconv source,
...
func atof(s *state, man uint64, exp10 int, neg bool) {
	...
	if f, ok := atof64exact(man, exp10, neg); ok {
		s.parseFloat(f)
		return
	}
	...
	// fixme: number is out of range!
	// we need arbitrary-precision decimal parsing
	// in order for the rounding to be done correctly...
	// but, we will only hit this when the source data
	// wasn't originally a float64, so is it really a common problem?
	s.parseFloat(math.NaN())
}
