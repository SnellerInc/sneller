...
func rewrite1(p *prog, v *value) (*value, bool) {
	...
	case scvtktoi: /* cvt.k@i */
		if len(v.args) == 2 {
			...
			if _tmp30 := v.args[0]; _tmp30.op == 1 /* init */ {
				return /* clobber v */ p.setssa(v, 154 /* broadcast.i */, 1), true
			}
			// (cvt.k@i (false) _) -> (broadcast.i 0)
			if _tmp31 := v.args[0]; _tmp31.op == 6 /* false */ {
				return /* clobber v */ p.setssa(v, 154 /* broadcast.i */, 0), true
			}
		}
	case scvtktof: /* cvt.k@f */
		if len(v.args) == 2 {
			...
			if _tmp32 := v.args[0]; _tmp32.op == 1 /* init */ {
				return /* clobber v */ p.setssa(v, 153 /* broadcast.f */, 1), true
			}
			// (cvt.k@f (false) _) -> (broadcast.f 0)
			if _tmp33 := v.args[0]; _tmp33.op == 6 /* false */ {
				return /* clobber v */ p.setssa(v, 153 /* broadcast.f */, 0), true
			}
		}
	case scvtitok: /* cvt.i@k */
		if len(v.args) == 2 {
			// (cvt.i@k _tmp0:(broadcast.i imm) k) -> (and.k "p.choose(imm != 0)" k)
			if _tmp0 := v.args[0]; _tmp0.op == 154 /* broadcast.i */ {
				if k := v.args[1]; true {
	...
	case sconcatstr2: /* concat2.str */
		if len(v.args) == 3 {
			...
			if x := v.args[0]; true {
				if _tmp1 := v.args[1]; _tmp1.op == 129 /* literal */ {
					if k := v.args[2]; true {
						if _tmp1.imm == "" {
							if p.mask(x) == k {
								return x, true
	...
	case sconcatstr3: /* concat3.str */
		if len(v.args) == 4 {
			...
			if _tmp4 := v.args[0]; _tmp4.op == 129 /* literal */ {
				if x := v.args[1]; true {
					if y := v.args[2]; true {
						if k := v.args[3]; true {
							if _tmp4.imm == "" {
			...
			if x := v.args[0]; true {
				if y := v.args[1]; true {
					if _tmp5 := v.args[2]; _tmp5.op == 129 /* literal */ {
						if k := v.args[3]; true {
							if _tmp5.imm == "" {
								return /* clobber v */ p.setssa(v, 81 /* concat2.str */, nil, x, y, k), true
							}
			...
			if x := v.args[0]; true {
				if _tmp6 := v.args[1]; _tmp6.op == 129 /* literal */ {
					if y := v.args[2]; true {
						if k := v.args[3]; true {
							if _tmp6.imm == "" {
								return /* clobber v */ p.setssa(v, 81 /* concat2.str */, nil, x, y, k), true
	...
	case sconcatstr4: /* concat4.str */
		if len(v.args) == 5 {
			...
			if x := v.args[0]; true {
				if y := v.args[1]; true {
					if z := v.args[2]; true {
						if _tmp7 := v.args[3]; _tmp7.op == 129 /* literal */ {
							if k := v.args[4]; true {
								if _tmp7.imm == "" {
				...
				if ov := v.args[1]; true {
					if k := v.args[2]; k.op == 6 /* false */ {
						if slot := v.imm.(int); true {
							if ov != k {
								return /* clobber v */ p.setssa(v, 135 /* store.z */, slot, mem, k, k), true
							}
						}
					}
				}
	...
	case saddf: /* add.f */
		if len(v.args) == 3 {
			...
			if f := v.args[0]; true {
				if _tmp8 := v.args[1]; _tmp8.op == 153 /* broadcast.f */ {
					if k := v.args[2]; true {
						if imm := tof64(_tmp8.imm); true {
							return /* clobber v */ p.setssa(v, 189 /* add.imm.f */, imm, f, k), true
						}
					}
				}
			}
			// (add.f _tmp9:(broadcast.f imm) f k) -> (add.imm.f f k imm)
			if _tmp9 := v.args[0]; _tmp9.op == 153 /* broadcast.f */ {
				if f := v.args[1]; true {
					if k := v.args[2]; true {
	...
	case ssubf: /* sub.f */
		if len(v.args) == 3 {
			...
			if f := v.args[0]; true {
				if _tmp10 := v.args[1]; _tmp10.op == 153 /* broadcast.f */ {
					if k := v.args[2]; true {
						if imm := tof64(_tmp10.imm); true {
							return /* clobber v */ p.setssa(v, 193 /* sub.imm.f */, imm, f, k), true
						}
					}
				}
			}
			// (sub.f _tmp11:(broadcast.f imm) f k) -> (rsub.imm.f f k imm)
			if _tmp11 := v.args[0]; _tmp11.op == 153 /* broadcast.f */ {
				if f := v.args[1]; true {
					if k := v.args[2]; true {
						if imm := tof64(_tmp11.imm); true {
							return /* clobber v */ p.setssa(v, 197 /* rsub.imm.f */, imm, f, k), true
						}
					}
				}
			}
	...
	case srsubimmf: /* rsub.imm.f */
		if len(v.args) == 2 {
			...
			if f := v.args[0]; true {
				if k := v.args[1]; true {
					if tof64(v.imm) == 0 {
						return /* clobber v */ p.setssa(v, 157 /* neg.f */, nil, f, k), true
					}
				}
			}
		}
	case srsubimmi: /* rsub.imm.i */
		if len(v.args) == 2 {
			...
			if i := v.args[0]; true {
				if k := v.args[1]; true {
					if toi64(v.imm) == 0 {
						return /* clobber v */ p.setssa(v, 158 /* neg.i */, nil, i, k), true
					}
				}
			}
		}
	case smulf: /* mul.f */
		if len(v.args) == 3 {
			...
			if f := v.args[0]; true {
				if _tmp12 := v.args[1]; _tmp12.op == 153 /* broadcast.f */ {
					if k := v.args[2]; true {
						if imm := tof64(_tmp12.imm); true {
							return /* clobber v */ p.setssa(v, 201 /* mul.imm.f */, imm, f, k), true
						}
					}
				}
			}
			// (mul.f _tmp13:(broadcast.f imm) f k) -> (mul.imm.f f k imm)
			if _tmp13 := v.args[0]; _tmp13.op == 153 /* broadcast.f */ {
				if f := v.args[1]; true {
					if k := v.args[2]; true {
	...
	case sdivf: /* div.f */
		if len(v.args) == 3 {
			...
			if f := v.args[0]; true {
				if _tmp14 := v.args[1]; _tmp14.op == 153 /* broadcast.f */ {
					if k := v.args[2]; true {
						if imm := tof64(_tmp14.imm); true {
							return /* clobber v */ p.setssa(v, 205 /* div.imm.f */, imm, f, k), true
						}
					}
				}
			}
			// (div.f _tmp15:(broadcast.f imm) f k) -> (rdiv.imm.f f k imm)
			if _tmp15 := v.args[0]; _tmp15.op == 153 /* broadcast.f */ {
				if f := v.args[1]; true {
					if k := v.args[2]; true {
						if imm := tof64(_tmp15.imm); true {
							return /* clobber v */ p.setssa(v, 209 /* rdiv.imm.f */, imm, f, k), true
						}
					}
				}
			}
	...
	case sboxint: /* boxint */
		if len(v.args) == 2 {
			...
			if _tmp16 := v.args[0]; _tmp16.op == 154 /* broadcast.i */ {
				if lit := toi64(_tmp16.imm); true {
					return /* clobber v */ p.setssa(v, 129 /* literal */, lit), true
				}
			}
		}
	case sboxfloat: /* boxfloat */
		if len(v.args) == 2 {
			...
			if _tmp17 := v.args[0]; _tmp17.op == 153 /* broadcast.f */ {
	...
	case sboxts: /* boxts */
		if len(v.args) == 2 {
			...
			if _tmp18 := v.args[0]; _tmp18.op == 278 /* broadcast.ts */ {
				if lit := toi64(_tmp18.imm); true {
					if ts := date.UnixMicro(int64(lit)); true {
						return /* clobber v */ p.setssa(v, 129 /* literal */, ts), true
					}
				}
			}
		}
