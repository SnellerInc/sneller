WITH
  "$source" AS
    (SELECT *
     ...
     WHERE (("$source"."timestamp" >= `2023-03-01T00:00:00Z`) AND ("$source"."timestamp" <= `2023-07-01T00:00:00Z`))
    ),

  "$bucket:events_over_time%0" AS
    (SELECT TIME_BUCKET("$source"."timestamp",86400) AS "$key:events_over_time%0",
            COUNT(*) AS "$doc_count",
            COUNT(*) FILTER (WHERE ("$source"."OriginWeather" = 'unknown')) AS "unknown_weather:$doc_count"
     FROM "$source"
