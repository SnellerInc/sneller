...
// runeToUtf8Array encodes the provided rune to UTF8 in a byte array of length 4
...
// to4ByteArray encode provided int to uint32 with little endian encoding in a byte array (of len 4)
...
func stringAlternatives(str string, reversed, addASCII, addLowerBounds, addUpperBounds bool) (alt []byte) {
	...
	byteLengthBounds := func(pos int, runeAlternatives [][4]rune) (lowerBound, upperBound int) {
		for i := pos; i < len(runeAlternatives); i++ {
			nBytes0 := utf8.RuneLen(runeAlternatives[i][0])
			nBytes1 := utf8.RuneLen(runeAlternatives[i][1])
			nBytes2 := utf8.RuneLen(runeAlternatives[i][2])
			nBytes3 := utf8.RuneLen(runeAlternatives[i][3])
			upperBound += max(nBytes0, nBytes1, nBytes2, nBytes3)
			lowerBound += min(nBytes0, nBytes1, nBytes2, nBytes3)
		}
		return lowerBound, upperBound
	}

	// alternativeRune gives all case-insensitive alternatives for the provided rune
	alternativeRune := func(r0 rune) [4]rune {
		r1 := unicode.SimpleFold(r0)
		if r1 == r0 {
			return [4]rune{r0, r0, r0, r0}
		}
		r2 := unicode.SimpleFold(r1)
		if (r2 == r0) || (r2 == r1) {
			x := []rune{r0, r1}
			slices.Sort(x)
			return [4]rune{x[0], x[1], x[1], x[1]}
		}
		r3 := unicode.SimpleFold(r2)
		if (r3 == r0) || (r3 == r1) || (r3 == r2) {
			x := []rune{r0, r1, r2}
			slices.Sort(x)
			return [4]rune{x[0], x[1], x[2], x[2]}
		}
		x := []rune{r0, r1, r2, r3}
		slices.Sort(x)
		return ([4]rune)(x)
	...
	// reverseSlice reversed the provided slide
	reverseSlice := func(slice []int) {
	...
	runes := []rune(str)
	runeAlternatives := make([][4]rune, nRunes)
	upperRunes := make([]rune, nRunes)

	for i, r := range runes {
		runeAlternatives[i] = alternativeRune(r)
		upperRunes[i] = min(runeAlternatives[i][0], runeAlternatives[i][1], runeAlternatives[i][2], runeAlternatives[i][3])
	}
	...
	for i := 0; i < nRunes; i++ {
		if reversed {
			...
			alt = append(runeToUtf8Array(runeAlternatives[i][0]), alt...)
			alt = append(runeToUtf8Array(runeAlternatives[i][1]), alt...)
			alt = append(runeToUtf8Array(runeAlternatives[i][2]), alt...)
			alt = append(runeToUtf8Array(runeAlternatives[i][3]), alt...)
		} else {
			alt = append(alt, runeToUtf8Array(runeAlternatives[i][0])...)
			alt = append(alt, runeToUtf8Array(runeAlternatives[i][1])...)
			alt = append(alt, runeToUtf8Array(runeAlternatives[i][2])...)
			alt = append(alt, runeToUtf8Array(runeAlternatives[i][3])...)
			if addASCII {
...
// e.g. "a__b", (with WC = '_') becomes ["a", "__", "b"], [[false], [true, true], [false]]
