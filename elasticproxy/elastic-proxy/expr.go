...
// exprSources represents a table source
type exprSources struct {
	Context *QueryContext
	Sources []TableSource
}

func (e *exprSources) QueryContext() *QueryContext {
	return e.Context
}

func (e *exprSources) Print(pc *printContext) {
	if len(e.Sources) > 1 {
		pc.WriteRune('(')
	}
	for i, s := range e.Sources {
		...
		if s.Database != "" {
			pc.WriteString(fmt.Sprintf("%q.", s.Database))
		}
		pc.WriteString(fmt.Sprintf("%q", s.Table))
	}
	if len(e.Sources) > 1 {
		pc.WriteRune(')')
	}
