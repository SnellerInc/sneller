...
import (
	...
	"github.com/SnellerInc/sneller/elasticproxy/helpers"
...
func TestAggregationsWithMultipleSources(t *testing.T) {
	...
	expected := `
		WITH
		  "$source" AS
			(SELECT *
			FROM ("db1"."tableX" ++ "db2"."tableY") AS "$source"
			)
	...
	qc := QueryContext{
		...
		TableSources: []TableSource{
			{Database: "db1", Table: "tableX"},
			{Database: "db2", Table: "tableY"},
		},
		IgnoreTotalHits: false,
		TypeMapping: map[string]TypeMapping{
			"timestamp": {
	...
	sql, err := ej.SQL(&qc)
	if err != nil {
		t.Fatalf("can't transform aggregation %q: %v", input, err)
	}
...
func runTest(t *testing.T, folder, database, table, index string, typeMapping map[string]TypeMapping) {
	...
	// Override end-points with local versions
	env.Elasticsearch.Endpoint = "http://localhost:9200"
	env.Sneller.Endpoint = "http://localhost:9180"

	// Check if actual Elastic instance can be reached
	...
	if err = hostReachable(env.Elasticsearch.Endpoint); err == nil {
		...
		t.Logf("testing against actual Elastic Search endpoint %q", env.Elasticsearch.Endpoint)
	} else {
		t.Logf("Elastic Search not available at %q, so testing with previous cached results.", env.Elasticsearch.Endpoint)
	}

	testLocalSnellerd := false
	if err = hostReachable(env.Sneller.Endpoint); err == nil {
		t.Logf("testing against actual Sneller endpoint %q", env.Sneller.Endpoint)
	} else {
		t.Logf("Sneller not available at %q, so testing with previous cached results.", env.Sneller.Endpoint)
	}
	...
	for _, af := range aggsFiles {
		...
		t.Run(test, func(t *testing.T) {
			...
			qc := QueryContext{
				Query:           ej,
				TableSources:    []TableSource{{Database: database, Table: table}},
				Index:           index,
				IgnoreTotalHits: false,
			...
			if testLocalSnellerd {
				...
				if err != nil {
					t.Fatalf("endpoint %s: %v", s.Endpoint, err)
				}
				resp, err := ExecuteQuery(client, u, s.Token, gotSQL)
				if err != nil {
