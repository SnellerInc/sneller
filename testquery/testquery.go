...
// ShuffleOutput determines whether the output
...
// ShuffleSymtab determines whether symtab can
// be safely shuffled
...
// Shuffle shuffles the tc and return true if
// successful; false otherwise
func (q *TestCaseIon) Shuffle() bool {
	if !canShuffle(q.Query) {
		return false
	}
	...
	} else if in := q.Input[0]; ShuffleOutput(q.Query) && len(in) == len(q.Output) {
