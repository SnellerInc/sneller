...
func (q *QueueRunner) updateDefs(m map[dbtable]*tableInfo) error {
	...
	for i := range dbs {
		...
		// cleanup autogenerated root
		// definition.json per #1885
		//
		// TODO: remove this code after all root
		// definition files have been cleaned up
		rmfs, ok := dir.(RemoveFS)
		if ok {
			rmfs.Remove(path.Join(curp, "definition.json"))
		}
		for j := range tables {
